...
# checking if the argument is a simple string
	if ! command -v "$name" &> /dev/null; then
	  log_debug "Span name: $name (treated as a string)"
  ...
  if [ -z "${OTEL_LOG_LEVEL-}" ]; then
    ...
    echo "This is the sent data"
    echo ${otel_trace_resource_spans}
	else
    log_debug "[$( caller )] $*" >&2
